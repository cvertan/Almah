...
public class MorphoTranscrNode extends  LetterTreeNode {
	...
	public MorphoTranscrNode(PhonemNode node,PhonemNode nextNode,MorphoTranscrNode node1) {
		...
		if (s.length()>=1) {
		       ...
		       // else {super.setLevel(2); super.setFinish(true);}
		}      		
		//else {super.setLevel(2); super.setFinish(true);}
	}
	//MorphoTrancr elem
...
public boolean getAbstract() {
	 ...
	 public String calculateLabel() {
		 ...
		 if( children.isEmpty()) {
			 ...
			 for(int i=0;i<children.size();i++) {
				 if (children.get(i).getLevelL()==5) s=s+"";
				 else  s=s+children.get(i).getLabel();
			 }
			
		 }
		 if (getLevelL()==5) return this.label;
		 else return s;
	 }
	...
	public MorphoTranscrNode getOldParent() {
		return oldparent;
	}

	public void setOldParent(MorphoTranscrNode m) {
		oldparent = m;
	}

	public void setPhonemParent(PhonemNode p) {
		phonemParent = p;
	}
