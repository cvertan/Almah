		    ...
		    JMenuItem jmi2 = new JMenuItem("Delete");
		        ...
		        jmi1.addActionListener(new ActionListener() {
		        	...
		        	syntaxFrame= new ChildFrame("Morphological Glossing for "+label,Color.BLUE,WindowConstants.DISPOSE_ON_CLOSE);
             		  ...
             		  right= new JPanel();
		        		...
		        		assign.addActionListener(new ActionListener() {
		        			public void actionPerformed(ActionEvent e1) {
		        				TreePath selectionPath = treePOS.getSelectionPath();
		        				if (selectionPath == null || selectionPath.getParentPath() == null) {
		        					JOptionPane.showMessageDialog(syntaxFrame, "Please select a syntactic category first.");
		        					return;
		        				}
		        				String sp = selectionPath.getLastPathComponent().toString();
		        				String spp = selectionPath.getParentPath().getLastPathComponent().toString();
		        				String sf="";
		        				ArrayList<SyntacticAnnotation> synfeat=new ArrayList<SyntacticAnnotation>();
		        				if (selectedTable == null || selectedTable.getModel() == null) {
		        					JOptionPane.showMessageDialog(syntaxFrame, "No syntactic feature table is available for the selected category.");
		        					return;
		        				}
		        				int viewRow = selectedTable.getSelectedRow();
		        				if (viewRow < 0 || viewRow >= selectedTable.getRowCount()) {
		        					JOptionPane.showMessageDialog(syntaxFrame, "Please select a valid syntactic feature row.");
		        					return;
		        				}
		        				int row;
		        				try {
		        					row = selectedTable.convertRowIndexToModel(viewRow);
		        				} catch (IndexOutOfBoundsException ex) {
		        					JOptionPane.showMessageDialog(syntaxFrame, "The selected syntactic feature row is no longer valid. Please select it again.");
		        					return;
		        				}
		        				if (row < 0 || row >= selectedTable.getModel().getRowCount()
		        						|| selectedTable.getModel().getColumnCount() < 3) {
		        					JOptionPane.showMessageDialog(syntaxFrame, "The selected syntactic feature row is no longer valid. Please select it again.");
		        					return;
		        				}
		        				String key=""+selectedTable.getModel().getValueAt(row, 0)+selectedTable.getModel().getValueAt(row, 1)+selectedTable.getModel().getValueAt(row, 2);
    		         ...
    		         //   System.out.println("Abbrev "+ sff.getAbbreviation());
    		                    if (sff == null) {
    		                    	JOptionPane.showMessageDialog(syntaxFrame, "No syntactic feature found for the selected row. Please choose another row.");
    		                    	return;
    		                    }
    		                     sa=new SyntacticAnnotation(spp,sp,sff);     
    		               //   sa=new SyntacticAnnotation(""+selectedTable.getModel().getValueAt(row, 1));
    		                syntaxFrame.doDefaultCloseAction();
    		              DefaultListModel<String> dlm = (DefaultListModel) synList.getModel();
    		              int insertIndex = selind;
    		              if (insertIndex < 0) insertIndex = synList.getSelectedIndex();
    		              if (insertIndex < 0) insertIndex = dlm.size();
    		              if (insertIndex > dlm.size()) insertIndex = dlm.size();
    		              dlm.insertElementAt(sa.getSyntFeature().getAbbreviation(), insertIndex);
    		              if (insertIndex > listSyntFeatures.size()) {
    		              	listSyntFeatures.add(sa);
    		              } else {
    		              	listSyntFeatures.add(insertIndex, sa);
    		              }
		        ...
		        jmi2.addActionListener(new ActionListener() {
		        	public void actionPerformed(ActionEvent e) {
		        		...
		        		int selectedIndex = synList.getSelectedIndex();
