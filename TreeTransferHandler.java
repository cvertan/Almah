...
class TreeTransferHandler extends TransferHandler {
    ...
    public boolean canImport(TransferHandler.TransferSupport support) {
        ...
        // In the morphological editor a single letter must never become a
        // direct child of the root.  Dropping a letter on the root previously
        // created a new phonem-like node and made the model inconsistent.
        Object targetObject = target.getMyLetterTreeNode();
        Object firstObject = firstNode.getMyLetterTreeNode();
        if (targetObject instanceof MorphoTranscrNode
                && firstObject instanceof MorphoTranscrNode) {
            MorphoTranscrNode targetMorph = (MorphoTranscrNode) targetObject;
            MorphoTranscrNode firstMorph = (MorphoTranscrNode) firstObject;
            if (targetMorph.getLevel() == 0 && firstMorph.getLevel() >= 2) {
                return false;
            }
        }
        return true;
