...
public String calculateLabel() {
	...
	public  ArrayList<TreeNode> listNodes(){
		 ...
		 if(morphoTranscr!=null && morphoTranscr.size()>0) {
		     for(int i=0;i<morphoTranscr.size();i++) {
		         String morphoId = morphoTranscr.get(i);
		         if (tag != null && tag.mmt != null && tag.mt != null) {
		             Integer idx = tag.mmt.get(morphoId);
		             if (idx != null && idx.intValue() >= 0 && idx.intValue() < tag.mt.size()) {
		                 MorphoSyntTranslit morpho = tag.mt.get(idx.intValue());
		                 if (morpho != null && !t.contains(morpho)) {
		                     t.add(morpho);
		                 }
		             }
		         }
