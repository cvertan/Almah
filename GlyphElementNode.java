...
private ArrayList<Reading> readings;/*ok*/
private JLabel uri;
...
private String imageUri;
	...
	public GlyphElementNode(String s, GlyphNode p, ArrayList<Reading> r) {
	    ...
	    for (int i=0;i<r.size();i++) {
	    	Reading rc = new Reading(r.get(i).getReading(),r.get(i).getConfidence(),r.get(i).getTyp(),r.get(i).getState());
            rc.setSourceUri(r.get(i).getSourceUri());
            readings.add(rc);
	    }
	    }
	    alternatives=new HashMap<String,ArrayList<Reading>>();
...
public void setImageUri(String imageUri) {
	if (imageUri == null) this.imageUri = "";
	else this.imageUri = imageUri;
}
