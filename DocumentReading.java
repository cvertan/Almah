...
public class DocumentReading extends TreeNode {
	...
	public DocumentReading(OElement lesung, AllmahGUI in) {
		...
		this.interf=in;
            ...
            Object confObj = lesung.getProperty("confidence");
            if (confObj != null) {
                setInterpretationConfidence("" + confObj);
            }
	...
	public OElement toDB(OElement el) {
		ODatabaseSession db =  this.interf.getDatabase();
         ...
         el.setProperty("confidence", getInterpretationConfidence(), OType.INTEGER);
    ...
    public String getNodeLabel() {
        ...
        int confidence = getInterpretationConfidence();
        if (confidence > 0) {
        	String color = "#7a3db8";

        	return "<html>" + label
        	        + " <font color=\"" + color + "\"><i>[conf: "
        	        + confidence
        	        + "]</i></font></html>";
        }
        return label;
