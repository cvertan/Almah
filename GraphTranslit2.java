...
public class GraphTranslit2 extends TreeNode {
	...
	public GraphTranslit2 copy(String t) {
		...
		copyTranslit2.setPathImage(this.pathImage);
		copyTranslit2.setNoVariants(this.novariants);
		copyTranslit2.setInterpretationConfidence(this.getInterpretationConfidence());

		HashMap<GraphGT2Node, GraphGT2Node> nodeMap = new HashMap<GraphGT2Node, GraphGT2Node>();
		ArrayList<GraphGT2Node> newelem = new ArrayList<GraphGT2Node>();

		for (int i = 0; i < elem.size(); i++) {
			GraphGT2Node oldNode = elem.get(i);
			GraphGT2Node newNode = oldNode.copy();
			newelem.add(newNode);
			nodeMap.put(oldNode, newNode);
			copyTranslit2.getGraphGT2().addVertex(newNode);
		}

		Iterator<OperatorLink> edgeIterator = g.getEdges().iterator();
		while (edgeIterator.hasNext()) {
			OperatorLink edge = edgeIterator.next();
			GraphGT2Node oldSource = g.getSource(edge);
			GraphGT2Node oldDest = g.getDest(edge);
			GraphGT2Node newSource = nodeMap.get(oldSource);
			GraphGT2Node newDest = nodeMap.get(oldDest);
			if (newSource != null && newDest != null) {
				copyTranslit2.getGraphGT2().addEdge(edge.copy(), newSource, newDest);
			}
		}
	...
	public ArrayList<GT2Element> getGT2Elements(){
		ArrayList<GT2Element>elem1=new ArrayList<GT2Element>();
		for (int i=0;i<elem.size();i++)
			  elem1.add(elem.get(i).getGT2Element());
		return elem1;
	}
