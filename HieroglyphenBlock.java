...
public class HieroglyphenBlock extends TreeNode{
	...
	private String numerical;/*ok*/
	
	private boolean state; /*ok*/
	private ArrayList<GlyphElementNode> elemNodes;/*ok*/
	private Graph<GlyphNode,OperatorLink> g ;
	private Graph<GlyphNode,OperatorLink> t; 
	 private String parent;/*ok*/
    private ArrayList<String> ntr1;/*ok*/
    private AllmahGUI interf;
    private String imageURL;/*ok*/
    private ArrayList<Object> teile;/*ok*/
    private ArrayList<String> annotations;/*ok*/
   private JPanel pane;
   ...
   private Map<String,String> imageUris = new HashMap<String,String>();
    ...
    public void setImageUris(Map<String,String> uris) {
        imageUris.clear();
        if (uris != null) {
            imageUris.putAll(uris);
        }
        for (GlyphElementNode glyph : elemNodes) {
            String key = cleanGlyphKey(glyph.getLabel());
            String uri = imageUris.get(key);
            if (uri != null) {
                glyph.setImageUri(uri);
            }
        }
    }
    ...
    private JLabel cloneLabel(JLabel src) {
        if (src == null) return new JLabel(" ");
        JLabel copy = new JLabel(src.getText());
        copy.setIcon(src.getIcon());
        copy.setHorizontalAlignment(src.getHorizontalAlignment());
        copy.setVerticalAlignment(src.getVerticalAlignment());
	...
	public void calculateImage(Map<String,JLabel> pictures) {
		...
		if(numerical.equals("#")) { 
			pane.setSize(50,50);pane.add(cloneLabel(pictures.get("#")));
		}
		...
		//	if(elemNodes.get(i).getClass().equals("class allmahVer4.GlyphElementNode")) {
		 if( elemNodes.get(i).getLabel().equals ("?")){
		        ...
		        for (Map.Entry<String, ArrayList<Reading>> entry : elemNodes.get(i).getAlternatives().entrySet()) {
		           ...
		           paux.add(cloneLabel(pictures.get(entry.getKey())));
		        }
			...
			pane.add(cloneLabel(pictures.get(temp)));
		...
		for (int i=0;i<annotations.size();i++) {
			...
			if (a.getTyp().equals("onomastic")) {
				c=interf.colO; abr="O"+countO+" ";
		         ...
		         if (a.getActivity().getTitle()!=null) s=s+"   <u>A</u>";
		         if (a.getDedication().getTitle()!=null) s=s+"  <u>D</u>";
		         if (a.getPlace().getPlaceName()!=null) s=s+"  <u>P</u>";
		         if (a.getEpigraphicActor().getActorName()!=null) s=s+"  <u>EA</u>";
		         if (a.getEpigraphicGroup().getGroupName()!=null) s=s+"  <u>EG</u>";
		}
		}
		s="<html>"+s+"</html>";
	return s;
