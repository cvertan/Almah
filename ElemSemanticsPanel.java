...
JList semList;
	public ElemSemanticsPanel(String s, IDictionary dict, AllmahGUI interf,int fx,int fy) {
		...
		g2.gridy=3;
		 ...
		 for(int i=0;i<20;i++)
           ...
           semList.setVisibleRowCount(10);
           semList.setFixedCellWidth(220);
	//       semList.addListSelectionListener(this);
                    ...
                    WN.setSize(500, 250);
            		   ...
            		   selMeaning.addActionListener(new ActionListener() {
                            ...
                            ArrayList<String> selectW=new ArrayList<String>();
               				...
               				DefaultListModel dlm =
     	        			       (DefaultListModel) semList.getModel();
                                      int insertIndex = semList.getSelectedIndex();
                                      if (insertIndex < 0) {
                                          insertIndex = 0;
                                      }
                                      dlm.insertElementAt(sf, insertIndex);
                                      if (insertIndex + 1 < dlm.getSize()
                                              && dlm.get(insertIndex + 1).equals("                             ")) {
                                          dlm.remove(insertIndex + 1);
                                      }
                                      semList.setSelectedIndex(insertIndex);
                                      semList.revalidate();
                                      semList.repaint();
                                      p.revalidate();
                                      p.repaint();
               				  meaningFrame.doDefaultCloseAction(); 	        			    	 dlm.remove(semList.getSelectedIndex()+1);
	      ...
	      p. setBorder( BorderFactory.createEtchedBorder(EtchedBorder.RAISED));
	       JScrollPane semScrollPane = new JScrollPane(semList);
           semScrollPane.setPreferredSize(new Dimension(240, 220));
           semScrollPane.setMinimumSize(new Dimension(220, 180));
           p.add(semScrollPane,g2);
	       g2.anchor=GridBagConstraints.CENTER;
	       g2.gridwidth=1;
	       g2.gridy=4;
