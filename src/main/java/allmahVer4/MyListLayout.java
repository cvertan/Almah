...
public class MyListLayout<Phonemnode, OperatorLink> extends AbstractLayout<PhonemNode,OperatorLink> {

	private double radius;
	private List<PhonemNode> vertex_ordered_list;
	
    protected LoadingCache<PhonemNode, CircleVertexData> circleVertexDatas =
    	CacheBuilder.newBuilder().build(new CacheLoader<PhonemNode, CircleVertexData>() {
	    	public CircleVertexData load(PhonemNode vertex) {
	    		return new CircleVertexData();
	    	}
    });

	public MyListLayout(Graph<PhonemNode,OperatorLink> g) {
		super(g);
	}
	...
	public void setVertexOrder(Comparator<PhonemNode> comparator)
	{
	    if (vertex_ordered_list == null)
	        vertex_ordered_list = new ArrayList<PhonemNode>(getGraph().getVertices());
     ...
     */
	public void setVertexOrder(List<PhonemNode> vertex_list)
	{
	...
	public void initialize() 
	{
		...
		{
		    if (vertex_ordered_list == null) 
		        setVertexOrder(new ArrayList<PhonemNode>(getGraph().getVertices()));
			...
			if (radius <= 0) {
				int summe=0;
				for (PhonemNode v : vertex_ordered_list)
				{
				summe= summe+(v.getLabel().length()*20);
				}
			...
			for (PhonemNode v : vertex_ordered_list)
			{
                ...
                summe=restLength+(v.getLabel().length()*20);
	...
	protected CircleVertexData getCircleData(PhonemNode v) {
