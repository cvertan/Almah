...
public class MyFRLayout<V, E> extends AbstractLayout<V, E> implements IterativeContext {
    ...
    public MyFRLayout(Graph<V, E> g, Dimension d) {
        super(g, new MyLocationTransformer<V>(d), d);
        initialize();
	...
	public void setSize(Dimension size) {
		if(initialized == false) {
			setInitializer(new MyLocationTransformer<V>(size));
		}
		super.setSize(size);
    ...
    protected synchronized void calcPositions(V v, int i) {
   ...
   //xyd.setLocation(newXPos, 30);
        }
        else  xyd.setLocation(50, 50);
    }
