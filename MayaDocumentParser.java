...
Element  teiroot;
	...
	public ArrayList<HieroglyphenBlock> getMayaDocumentParts(){
		...
		for (Element divElement: divEls) {
			...
			for (Element abElement: abEls) {
				...
				while(abChildren.hasNext()) {
					...
					if( c.getName().equals("g")){
						...
						if (c.getAttributeValue("ref").indexOf("textgrid")>=0) {
						 ...
						 if (c.getAttributeValue("n") != null && s != null && !s.isEmpty()) {
							    imageUris.put(c.getAttributeValue("n"), s);
							}
							}
						...
						//	BufferedImage pictureScaled=null;
							...
							BufferedImage pictureScaled=null;
						try {
							IdiomTextGridObject ob=new IdiomTextGridObject("src/main/resources/idiomQueries-SECRET-prod.properties");
							// picture=ob.getTextGridObjectByUri(s);
							System.out.println ("SID for image "+ sid);
						...
						catch(Exception e1) {System.out.println("Cannot read picture "+ s);}		
		      ...
		      blocks.get(blocks.size()-1).setImageUris(imageUris);
	...
	public ArrayList<HieroglyphenBlock> getParsedBlocks() {
	    return blocks;
	}
